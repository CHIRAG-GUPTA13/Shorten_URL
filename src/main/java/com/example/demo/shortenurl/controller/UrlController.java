...
public class UrlController {
     ...
     * Body: { "originalUrl": "https://example.com", "expiryDate":
     * "2024-12-31T23:59:59" }
     */
    @PostMapping("/shorten")
    ...
    /**
     * Shorten a URL with custom short code (Protected - requires JWT
     * authentication)
    ...
    /**
     * Get all active URLs for the authenticated user (Protected - requires JWT
     * authentication)
    ...
    /**
     * Get summary statistics for all URLs owned by the authenticated user
     * (Protected)
     * GET /api/urls/my-urls/stats/summary
     * Returns: { totalUrls, totalClicks, activeLinks, expiredLinks }
     */
    @GetMapping("/my-urls/stats/summary")
    public ApiResponse<UrlSummaryStatsDto> getMyUrlsStatsSummary() {
        logger.info("GET /api/urls/my-urls/stats/summary - Request received");

        // Extract user from SecurityContextHolder
        User currentUser = getCurrentUser();

        if (currentUser == null) {
            logger.warn("GET /api/urls/my-urls/stats/summary - User not authenticated");
            return ApiResponse.error(401, "User not authenticated");
        }

        logger.debug("GET /api/urls/my-urls/stats/summary - Processing request for userId: {}", currentUser.getId());

        try {
            UrlSummaryStatsDto stats = urlService.getUrlSummaryStats(currentUser);

            logger.info("GET /api/urls/my-urls/stats/summary - Found stats for userId: {}", currentUser.getId());
            return ApiResponse.success("Summary stats retrieved successfully", stats);

        } catch (Exception e) {
            logger.error("GET /api/urls/my-urls/stats/summary - Error processing request", e);
            throw e;
        }
    }

    /**
     * Get full metadata for a specific URL including click count and owner info
     * (Protected)
     * GET /api/urls/{shortCode}/metadata
     * Returns: UrlResponseDto with all fields including clickCount and ownerInfo
     */
    @GetMapping("/{shortCode}/metadata")
    public ApiResponse<UrlFullResponseDto> getUrlFullMetadata(@PathVariable String shortCode) {
        logger.info("GET /api/urls/{}/metadata - Request received", shortCode);

        // Extract user from SecurityContextHolder
        User currentUser = getCurrentUser();

        if (currentUser == null) {
            logger.warn("GET /api/urls/{}/metadata - User not authenticated", shortCode);
            return ApiResponse.error(401, "User not authenticated");
        }

        logger.debug("GET /api/urls/{}/metadata - Processing request for userId: {}", shortCode, currentUser.getId());

        try {
            ApiResponse<UrlFullResponseDto> response = urlService.getUrlFullMetadata(shortCode, currentUser);

            logger.info("GET /api/urls/{}/metadata - Completed with status code: {}", shortCode, response.getCode());
            return response;

        } catch (Exception e) {
            logger.error("GET /api/urls/{}/metadata - Error processing request", shortCode, e);
            throw e;
        }
    }
}
